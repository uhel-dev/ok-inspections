...
import PageLayout from "../mainLayout/PageLayout";
import AddDaysItems from "../add-items/AddDaysItems";
import AddChecklistItems from "../add-items/AddChecklistItems";
import AddAdditionalInformationItem from "../add-items/AddAdditionalInformationItem";
...
const template = {
  general: ["Site", "Serial No (s)", "Model", "Week commencing", "Manufacturer", "Last Examination", "SWL"],
  notes: ""
}


export default function CherryPickerTemplate() {
  const [general, setGeneral] = useState<any>(() => {
    return template.general.map((item: any) => {
      return {
        name: item,
        value: ''
      }
      ...
      <section className={`kdev-section`}>
        <div className={`grid grid-cols-3 gap-2`}>
          { general.map((item: any, index: number) => {
            return (
              <div className={index === 0 ? `col-span-3` : `col-span-1`}>
          ...
          <div className={`col-span-3`}>
            ...
            <input
              ...
              onChange={(e) => updateFormName(e.target.value)}
              type="text"
              className={`w-full p-2 border border-gray-200 min-h-[120px]`}
              required
            />
          </div>
        </div>
