...
export const MinifiedRequestCallbackDialog = (props: any) => {
    const heading = props.heading || "SELECT BEST TIME"
    const headingStyle = props.headingStyle || "btn over:text-black font-uk-reg-plates w-full"
    ...
    const sendRequestToTelegramBot = async () => {
      if (validPhoneNumber && validPostCode) {
        ...
        const callbackRequest = {
          ...
          location: requestCallbackPostCode
        };
        ...
        if (error) {
          ...
          trackEvent('telegram_message', 'Contact Request', 'telegram_message', {
            phoneNumber: requestCallbackPhoneNumber,
            location: requestCallbackPostCode
          });
          setRequestCallbackPhoneNumber('')
          setRequestCallbackPostCode('')
          setIsLoading(false)
          const el: any = document.getElementById("my-modal-7");
          el.checked = false;
        }
      }
    };
    ...
    return (
      <>
        <label htmlFor="my-modal-7" className={`${headingStyle} ${withIcon ? "flex items-center justify-center gap-2" : ""}`}>
          { withIcon && (
        ...
        <div id={`request-callback-modal`} className="modal modal-bottom sm:modal-middle">
          <div className="modal-box flex flex-col gap-2">
            <div className={`flex justify-between items-center gap-2`}>
              <div className="font-bold text-sm lg:text-lg font-uk-reg-plates">Thank You! Let&apos;s Secure Your Tyre Size & Callback</div>
            ...
            <div className="modal-action">
              <button
                ...
                onClick={() => sendRequestToTelegramBot()}
              >
                { !isLoading && (
                  <>
                    Send Request
